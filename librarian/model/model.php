...
function updateProfilePicture($username, $file){
    $conn = db_conn();
    $selectQuery = "UPDATE librarian set IMAGE = '$file' where USERNAME = '$username'";
    try{
          $stmt = $conn->query($selectQuery);
...
function showAllBook(){
	$conn = db_conn();
    $selectQuery = 'SELECT * FROM `borrowbook` ';
    try{
        $stmt = $conn->query($selectQuery);
    }catch(PDOException $e){
        echo $e->getMessage();
...
function show($username){
	$conn = db_conn();
	$selectQuery = "SELECT * FROM `librarian` where USERNAME = '$username'";
...
function addRegistrationInfo($data){
	$conn = db_conn();
    $selectQuery = "INSERT into librarian (NAME, EMAIL,USERNAME,PASSWORD,PHONE,LOCATION)
...
function addBookInfo($data){
	$conn = db_conn();
    $selectQuery = "INSERT into book (NAME, CODE, AMOUNT)
...
function update($username, $data){
    $conn = db_conn();
    $selectQuery = "UPDATE librarian set NAME = ?, EMAIL = ?, PHONE = ? where USERNAME = '$username'";
    try{
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
...
function updatePassword($username, $password){
    $conn = db_conn();
    $selectQuery = "UPDATE librarian set PASSWORD = '$password' where USERNAME = '$username'";
    try{
          $stmt = $conn->query($selectQuery);
